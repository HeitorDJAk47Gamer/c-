...
## 2024-05-19 - Localized Decimal Separators in CLI Inputs
**Learning:** In localized environments like Portuguese, users intuitively use commas for decimals. Failing to handle this in CLI apps using sscanf truncates input (e.g., 7,5 becomes 7.0), confusing users. Intercepting and normalizing commas to dots significantly improves the perceived robustness and friendliness of the tool.
**Action:** Always intercept and normalize localized decimal separators (like replacing commas with dots) before parsing floating-point inputs in C/C++ CLI applications to prevent unexpected truncation and gracefully handle user habits.
